...
# === ОЦЕНКА ===
def eval_run(ids: list[str], gt: set[str]) -> tuple[float,float,float]:
    if not ids:
        return 0.0, 0.0, 0.0
...
if __name__ == "__main__":
    ...
    for q in queries:
        ...
        for label, fields in field_sets:
            ...
            # собираем метрики для каждого режима
            for name, expand, norm in modes:
                ids = retrieve(q, expand, norm, fields)
                p, r, f = eval_run(ids, gt)
                logging.info(f"{name:<10} → Ret={len(ids):>3}, P={p:.3f}, R={r:.3f}, F1={f:.3f}")
    ...
    # записываем в JSON
    logging.info(f"Сохранение результатов в {OUTPUT_PATH}")
